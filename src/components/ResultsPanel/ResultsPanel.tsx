...
export const ResultsPanel: React.FC<ResultsPanelProps> = ({ analysis, isLocked }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Building Info */}
      <div className="bg-[#007A41]/10 rounded-lg p-4 flex items-center gap-3">
        <div className="p-2 bg-[#007A41] rounded-lg text-white">
          {getBuildingIcon()}
        </div>
        <div>
          <p className="text-sm text-gray-600">Building Type</p>
          <p className="font-semibold text-[#007A41] capitalize">
      ...
      <div className={`bg-white border border-gray-200 rounded-lg p-4 ${isLocked ? 'opacity-50' : ''}`}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800 flex items-center gap-2">
            <Image className="w-5 h-5 text-[#FFC132]" />
            Symbol Detection & Pricing
          </h3>
          {!isLocked && (
            <button
              onClick={handleAdd}
              className="flex items-center gap-1 text-xs bg-[#007A41] text-white px-2 py-1 rounded hover:bg-[#007A41]/90 transition-colors"
            >
                ...
                {editingId === match.id ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full text-sm border border-gray-300 rounded px-2 py-1 mt-1"
                        >
                          {Object.entries(COMPONENT_PRICES).map(([type, _]) => (
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                      <span className="font-semibold text-[#007A41]">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-gray-600">
                        <span>Count: <strong>{match.count}</strong></span>
                        <span>Price: <strong>{formatCurrency(match.unitPrice)}</strong></span>
                        <span>Rating: <strong>{match.rating} kW</strong></span>
                      </div>
                    </div>
        ...
        {symbolMatches.length > 0 && (
          <div className="mt-4 pt-4 border-t-2 border-gray-200">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xl font-bold text-[#007A41]">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
          <Zap className="w-5 h-5 text-[#FFC132]" />
          Load Summary
        </h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-xs text-gray-600">Total Connected Load</p>
            <p className="text-xl font-bold text-[#007A41]">
      ...
      <div className={`bg-white border border-gray-200 rounded-lg p-4 ${isLocked ? 'opacity-50' : ''}`}>
        <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
          <Shield className="w-5 h-5 text-[#FFC132]" />
          Redundancy Planning
        </h3>
        <div className="space-y-3">
      ...
      {/* Compliance Checks */}
      <div className={`bg-white border border-gray-200 rounded-lg p-4 ${isLocked ? 'opacity-50' : ''}`}>
        <h3 className="font-semibold text-gray-800 mb-4">GS1009 Compliance</h3>
        <div className="space-y-2">
          {analysis.complianceChecks.map((check, index) => (
            <div key={index} className="flex items-start gap-2 p-2 bg-gray-50 rounded-lg">
              {check.passed ? (
                <CheckCircle className="w-4 h-4 text-[#007A41] mt-0.5" />
              ) : (
                <AlertCircle className="w-4 h-4 text-red-500 mt-0.5" />
              )}
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-800">{check.standard}</p>
      ...
      <div className={`bg-white border border-gray-200 rounded-lg p-4 ${isLocked ? 'opacity-50' : ''}`}>
        <h3 className="font-semibold text-gray-800 mb-4">Power Sourcing Recommendations</h3>
        <div className="space-y-3">
          {analysis.recommendations.map((rec, index) => (
            <div key={index} className="p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-[#007A41]">{rec.percentage}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                <div 
                  className="bg-[#FFC132] h-2 rounded-full"
      ...
      {/* Summary */}
      <div className={`bg-[#007A41] text-white rounded-lg p-4 ${isLocked ? 'opacity-50' : ''}`}>
